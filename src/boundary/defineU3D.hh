...
void defineUBouzidi(BlockLatticeStructure3D<T,DESCRIPTOR>& _block, int iX, int iY, int iZ, int iPop, const T u[DESCRIPTOR::d])
{
  ...
  bool _output = false;
  //std::cout << _block.getDynamics(iX, iY, iZ)->getName() << std::endl;
...
void defineConcentrationBouzidi(SuperLattice3D<T,DESCRIPTOR>& sLattice, SuperGeometry3D<T>& superGeometry, int material,
                    AnalyticalF3D<T,T>& rho, std::vector<int> bulkMaterials)
{
  defineConcentrationBouzidi<T,DESCRIPTOR>(sLattice, superGeometry.getMaterialIndicator(material), rho, bulkMaterials);
}
...
void defineConcentrationBouzidi(SuperLattice3D<T,DESCRIPTOR>& sLattice, FunctorPtr<SuperIndicatorF3D<T>>&& boundaryIndicator,
                    AnalyticalF3D<T,T>& rho, std::vector<int> bulkMaterials)
{
  defineConcentrationBouzidi<T,DESCRIPTOR>(sLattice, std::forward<decltype(boundaryIndicator)>(boundaryIndicator),
                               boundaryIndicator->getSuperGeometry().getMaterialIndicator(std::move(bulkMaterials)),
                               rho);
}
                    ...
                    FunctorPtr<SuperIndicatorF3D<T>>&& bulkIndicator, AnalyticalF3D<T,T>& rho)
{
  ...
  for (int iCloc = 0; iCloc < sLattice.getLoadBalancer().size(); ++iCloc) {
    defineConcentrationBouzidi<T,DESCRIPTOR>(sLattice.getExtendedBlockLattice(iCloc),boundaryIndicator->getExtendedBlockIndicatorF(iCloc),
                                 bulkIndicator->getExtendedBlockIndicatorF(iCloc),rho);
  }
          ...
          if (indicator(iX, iY, iZ)) {
            for (int q = 1; q < DESCRIPTOR::q ; ++q) {
              ...
              if (bulkIndicator.getBlockGeometryStructure().isInside(iXn,iYn,iZn) &&
                ...
                if (getBoundaryIntersection<T,DESCRIPTOR>(_block, iX, iY, iZ, opp, intersection)) {
                  T concentration = T();
                  rho(&concentration, intersection);
                  defineConcentrationBouzidi<T,DESCRIPTOR>(_block, iX, iY, iZ, opp, concentration);
                }
              }
            }
          }
...
{
  OstreamManager clout(std::cout, "defineConcentrationBouzidi");
  bool _output = false;
  _block.getDynamics(iX, iY, iZ)->defineConcentration(iPop, rho);
  if (_output) {
    clout << "defineConcentrationBouzidi(" << iX << ", " << iY << ", " << iZ << " )" << std::endl;
  }
