...
void setZouHePressureBoundary(SuperLattice3D<T, DESCRIPTOR>& sLattice, T omega, FunctorPtr<SuperIndicatorF3D<T>>&& indicator)
{
  ...
  for (int iC = 0; iC < sLattice.getLoadBalancer().size(); ++iC) {
    setZouHePressureBoundary<T,DESCRIPTOR,MixinDynamics>(sLattice.getExtendedBlockLattice(iC), omega, indicator->getExtendedBlockIndicatorF(iC),includeOuterCells);
  }
