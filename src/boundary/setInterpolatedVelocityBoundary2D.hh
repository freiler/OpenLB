...
void setInterpolatedVelocityBoundary(SuperLattice2D<T, DESCRIPTOR>& sLattice, T omega, FunctorPtr<SuperIndicatorF2D<T>>&& indicator)
{
  ...
  for (int iCloc = 0; iCloc < sLattice.getLoadBalancer().size(); ++iCloc) {
    setInterpolatedVelocityBoundary<T,DESCRIPTOR,MixinDynamics>(sLattice.getExtendedBlockLattice(iCloc), omega,
...
template<typename T, typename DESCRIPTOR, class MixinDynamics>
void setInterpolatedVelocityBoundary(BlockLatticeStructure2D<T,DESCRIPTOR>& block, T omega,BlockIndicatorF2D<T>& indicator, bool includeOuterCells)
{
  OstreamManager clout(std::cout, "setInterpolatedVelocityBoundary");
      ...
      if (indicator(iX, iY)) {
        ...
        if (discreteNormal[0] == 0) {
          ...
          if (discreteNormal[1] == 1) {
            ...
            clout << "Could not setInterpolatedVelocityBoundary (" << iX
                  << ", " << iY << ")" << std::endl;
          }
        }
          ...
          if (discreteNormal[1] == 1) {
            if (discreteNormal[2] == 1) {
              ...
              clout << "Could not setInterpolatedVelocityBoundary ("
                    << iX << ", " << iY << ")" << std::endl;
            }
          }
          else if (discreteNormal[1] == -1) {
