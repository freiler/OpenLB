...
template<typename T, typename DESCRIPTOR>
void setPartialSlipBoundary(BlockLatticeStructure3D<T,DESCRIPTOR>& block,T tuner, BlockIndicatorF3D<T>& indicator, bool includeOuterCells)
{
  OstreamManager clout(std::cout, "setPartialSlipBoundary");
      ...
      for (int iZ = z0; iZ <= z1; iZ++) {
        if (indicator(iX, iY, iZ)) {
          if (tuner < 0. || tuner > 1.) {
            ...
            block.defineDynamics(iX, iY, iZ, &instances::getBounceBack<T, DESCRIPTOR>());//define dynamics on indicated cells
          }
          else {
