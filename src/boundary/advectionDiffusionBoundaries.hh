...
//==================================================================================================
//==================== For Enthalpy Advection Diffusion Boundary Condition ======================
//============================================================================================
...
template<typename T, typename DESCRIPTOR, typename Dynamics, int direction, int orientation>
AdvectionDiffusionEnthalpyBoundariesDynamics<T,DESCRIPTOR,Dynamics,direction,orientation>::
AdvectionDiffusionEnthalpyBoundariesDynamics( T omega_, Momenta<T,DESCRIPTOR>& momenta_, T T_s_, T T_l_, T cp_s_, T cp_l_, T lambda_s_, T lambda_l_, T l_)
  : TotalEnthalpyAdvectionDiffusionBGKdynamics<T, DESCRIPTOR>( omega_, momenta_, T_s_, T_l_, cp_s_, cp_l_, lambda_s_, lambda_l_, l_ )
  , boundaryDynamics(omega_, momenta_,T_s_, T_l_, cp_s_, cp_l_, lambda_s_, lambda_l_, l_)
{
  this->getName() = "AdvectionDiffusionEnthalpyBoundariesDynamics";
}

template<typename T, typename DESCRIPTOR, typename Dynamics, int direction, int orientation>
T AdvectionDiffusionEnthalpyBoundariesDynamics<T,DESCRIPTOR,Dynamics,direction,orientation>::
computeEquilibrium(int iPop, T rho, const T u[DESCRIPTOR::d], T uSqr) const
{
  //return lbHelpers<T,DESCRIPTOR>::equilibriumFirstOrder( iPop, rho, u );
...
template<typename T, typename DESCRIPTOR, typename Dynamics, int direction, int orientation>
void AdvectionDiffusionEnthalpyBoundariesDynamics<T,DESCRIPTOR,Dynamics,direction,orientation>::
collide(Cell<T,DESCRIPTOR>& cell,LatticeStatistics<T>& statistics)
{
  typedef DESCRIPTOR L;
  typedef lbHelpers<T,DESCRIPTOR> lbH;

  T dirichletEnthalpy = this->_momenta.computeRho(cell);
  auto u = cell.template getField<descriptors::VELOCITY>();
  ...
  if ((L::d == 3 && L::q == 7)||(L::d == 2 && L::q == 5)) {
    ...
    T difference = dirichletEnthalpy - (T) 1 - sum; // on cell there are non-shiftet values -> temperature has to be changed
...
//=================================================================
template<typename T, typename DESCRIPTOR, typename Dynamics, int xNormal, int yNormal, int zNormal>
AdvectionDiffusionEnthalpyCornerDynamics3D<T,DESCRIPTOR,Dynamics,xNormal,yNormal,zNormal>::AdvectionDiffusionEnthalpyCornerDynamics3D(
  T omega_, Momenta<T,DESCRIPTOR>& momenta_, T T_s_, T T_l_, T cp_s_, T cp_l_, T lambda_s_, T lambda_l_, T l_)
  : TotalEnthalpyAdvectionDiffusionBGKdynamics<T, DESCRIPTOR>( omega_, momenta_, T_s_, T_l_, cp_s_, cp_l_, lambda_s_, lambda_l_, l_ ),
    boundaryDynamics(omega_, momenta_,T_s_, T_l_, cp_s_, cp_l_, lambda_s_, lambda_l_, l_)
{
  this->getName() = "AdvectionDiffusionEnthalpyCornerDynamics3D";
}

template<typename T, typename DESCRIPTOR, typename Dynamics,  int xNormal, int yNormal, int zNormal>
T AdvectionDiffusionEnthalpyCornerDynamics3D<T,DESCRIPTOR,Dynamics,xNormal,yNormal,zNormal>::computeEquilibrium(int iPop, T rho, const T u[DESCRIPTOR::d], T uSqr) const
{
  //return lbHelpers<T, DESCRIPTOR>::equilibriumFirstOrder( iPop, rho, u );
  return boundaryDynamics.computeEquilibrium( iPop, rho, u , uSqr);
}
...
template<typename T, typename DESCRIPTOR, typename Dynamics,  int xNormal, int yNormal, int zNormal>
void AdvectionDiffusionEnthalpyCornerDynamics3D<T,DESCRIPTOR,Dynamics,xNormal,yNormal,zNormal>::collide(Cell<T,DESCRIPTOR>& cell,LatticeStatistics<T>& statistics)
{
  typedef DESCRIPTOR L;
  typedef lbHelpers<T,DESCRIPTOR> lbH;

  T enthalpy = this->_momenta.computeRho(cell);
  auto u = cell.template getField<descriptors::VELOCITY>();
  // I need to get Missing information on the corners !!!!
  std::vector<int> unknownIndexes = util::subIndexOutgoing3DonCorners<L,xNormal,yNormal,zNormal>();
  // here I know all missing and non missing f_i
...
template<typename T, typename DESCRIPTOR, typename Dynamics, int xNormal, int yNormal, int zNormal>
T AdvectionDiffusionEnthalpyCornerDynamics3D<T,DESCRIPTOR,Dynamics,xNormal,yNormal,zNormal>::getOmega() const
{
  return boundaryDynamics.getOmega();
}

template<typename T, typename DESCRIPTOR, typename Dynamics, int xNormal, int yNormal, int zNormal>
void AdvectionDiffusionEnthalpyCornerDynamics3D<T,DESCRIPTOR,Dynamics,xNormal,yNormal,zNormal>::setOmega(T omega_)
{
  boundaryDynamics.setOmega(omega_);
}
...
//=================================================================
template<typename T, typename DESCRIPTOR, typename Dynamics, int plane, int normal1, int normal2>
AdvectionDiffusionEnthalpyEdgesDynamics<T,DESCRIPTOR,Dynamics,plane,normal1, normal2>::AdvectionDiffusionEnthalpyEdgesDynamics(
  T omega_, Momenta<T,DESCRIPTOR>& momenta_, T T_s_, T T_l_, T cp_s_, T cp_l_, T lambda_s_, T lambda_l_, T l_)
  : TotalEnthalpyAdvectionDiffusionBGKdynamics<T, DESCRIPTOR>( omega_, momenta_, T_s_, T_l_, cp_s_, cp_l_, lambda_s_, lambda_l_, l_ ),
    boundaryDynamics(omega_, momenta_,T_s_, T_l_, cp_s_, cp_l_, lambda_s_, lambda_l_, l_)
{
  this->getName() = "AdvectionDiffusionEnthalpyEdgesDynamics";
}

template<typename T, typename DESCRIPTOR, typename Dynamics, int plane, int normal1, int normal2>
T AdvectionDiffusionEnthalpyEdgesDynamics<T,DESCRIPTOR,Dynamics,plane,normal1, normal2>::computeEquilibrium(int iPop, T rho, const T u[DESCRIPTOR::d], T uSqr) const
{
  //return lbHelpers<T,DESCRIPTOR>::equilibriumFirstOrder( iPop, rho, u );
  return boundaryDynamics.computeEquilibrium( iPop, rho, u, uSqr );
}
...
template<typename T, typename DESCRIPTOR, typename Dynamics,  int plane, int normal1, int normal2>
void AdvectionDiffusionEnthalpyEdgesDynamics<T,DESCRIPTOR,Dynamics,plane,normal1, normal2>::collide(Cell<T,DESCRIPTOR>& cell,LatticeStatistics<T>& statistics)
{
  typedef DESCRIPTOR L;
  typedef lbHelpers<T,DESCRIPTOR> lbH;
...
template<typename T, typename DESCRIPTOR, typename Dynamics, int plane, int normal1, int normal2>
T AdvectionDiffusionEnthalpyEdgesDynamics<T,DESCRIPTOR,Dynamics,plane,normal1, normal2>::getOmega() const
{
  return boundaryDynamics.getOmega();
}

template<typename T, typename DESCRIPTOR, typename Dynamics, int plane, int normal1, int normal2>
void AdvectionDiffusionEnthalpyEdgesDynamics<T,DESCRIPTOR,Dynamics,plane,normal1, normal2>::setOmega(T omega_)
{
  boundaryDynamics.setOmega(omega_);
}
