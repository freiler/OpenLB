...
void setRegularizedTemperatureBoundary(SuperLattice2D<T, DESCRIPTOR>& sLattice,T omega,
                                       ...
                                       FunctorPtr<SuperIndicatorF2D<T>>&& indicator)
{
  ...
  for (int iCloc = 0; iCloc < sLattice.getLoadBalancer().size(); ++iCloc) {
    setRegularizedTemperatureBoundary<T,DESCRIPTOR,MixinDynamics>(sLattice.getExtendedBlockLattice(iCloc),omega,
  ...
  for (int iX = x0; iX <= x1; ++iX) {
    for (int iY = y0; iY <= y1; ++iY) {
      ...
      if (indicator(iX, iY)) {
        ...
        if (discreteNormal[0] == 0) {   ////set momenta, dynamics and postProcessors on indicated RegularizedTemperatureBoundary cells
          if (discreteNormal[1] == 1) {
            momenta = new RegularizedTemperatureBM<T,DESCRIPTOR,0,1>;
            ...
            momenta = new RegularizedTemperatureBM<T,DESCRIPTOR,1,1>;
            dynamics = new CombinedAdvectionDiffusionRLBdynamics<T,DESCRIPTOR,MixinDynamics>(omega, *momenta);
            postProcessor = nullptr;
            setBoundary<T, DESCRIPTOR, MixinDynamics>(block, omega, iX,iY, momenta, dynamics, postProcessor);
        ...
        else if (discreteNormal[0] == 1) {////set momenta, dynamics and postProcessors on indicated RegularizedTemperatureBoundary corner cells
