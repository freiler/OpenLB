...
#ifndef LinearContactForce3D_HH
#define LinearContactForce3D_HH
...
LinearContactForce3D<T, PARTICLETYPE, DESCRIPTOR>::LinearContactForce3D(
...
template<typename T, template<typename U> class PARTICLETYPE, template<
           typename W> class DESCRIPTOR>
void LinearContactForce3D<T, PARTICLETYPE, DESCRIPTOR>::applyForce(
...
template<typename T, template<typename U> class PARTICLETYPE, template<
           typename W> class DESCRIPTOR>
void LinearContactForce3D<T, PARTICLETYPE, DESCRIPTOR>::computeForce(
  typename std::deque<PARTICLETYPE<T> >::iterator p, int pInt,
  ...
  for (const auto& it : ret_matches) {
    ...
    if (!util::nearZero(it.second)) {
      ...
      if (delta > 0.) {
        ...
        T A = M_PI * std::pow(min(p->getRad(),p2->getRad()), 2.);
        T kn = pow(p->getRad()/(A*E1) + p2->getRad()/(A*E2) , -1.);
