...
template<typename T>
CuboidNeighbourhood2D<T>::CuboidNeighbourhood2D(SuperStructure2D<T>& superStructure, int iC):
  ...
  _iCblock(_iCuboid.getNx(), _iCuboid.getNy(), _superStructure.getOverlap())
{
  _initInCNdone  = false;
...
template<typename T>
CuboidNeighbourhood2D<T>::CuboidNeighbourhood2D(CuboidNeighbourhood2D<T> const& rhs):
...
template<typename T>
std::uint8_t** CuboidNeighbourhood2D<T>::get_inData()
{
  return _inData;
}

template<typename T>
std::uint8_t** CuboidNeighbourhood2D<T>::get_outData()
{
  return _outData;
}
...
template<typename T>
std::size_t CuboidNeighbourhood2D<T>::getLocalCellId(int iX, int iY) const
{
  const int overlap = _superStructure.getOverlap();
  return _iCblock.getCellId(iX+overlap, iY+overlap);
}

template<typename T>
void CuboidNeighbourhood2D<T>::add_inCell(Cell2D<T> cell)
{
  cell.latticeCellId = getLocalCellId(cell.latticeR[1], cell.latticeR[2]);
...
void CuboidNeighbourhood2D<T>::add_inCell(int iX, int iY)
{
  ...
  std::vector<T> tmp(found.physR,found.physR + 2);
...
#ifdef PARALLEL_MODE_MPI
template<typename T>
void CuboidNeighbourhood2D<T>::bufSend_inCells(singleton::MpiNonBlockingHelper& helper)
{
  ...
  for (unsigned iC=0; iC<_inC.size(); iC++) {
    ...
    singleton::mpi().iSend(_inDataCoordinates[_inC[iC]],
                           _inN[iC]*2, dRank, helper.get_mpiRequest(iC), _iCglob);
  }
}
#endif
