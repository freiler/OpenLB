...
#ifndef BLOCK_STATIC_POPULATION_D_3D_HH
#define BLOCK_STATIC_POPULATION_D_3D_HH
...
BlockStaticPopulationD3D<T,DESCRIPTOR>::BlockStaticPopulationD3D(int nX, int nY, int nZ):
...
template<typename T, typename DESCRIPTOR>
T* BlockStaticPopulationD3D<T,DESCRIPTOR>::getPopulationPointer(unsigned iPop, std::size_t iCell) {
  return (iCell > _remainder[iPop] ? _population[iPop][1] : _population[iPop][0]) + iCell;
}

template<typename T, typename DESCRIPTOR>
const T* BlockStaticPopulationD3D<T,DESCRIPTOR>::getPopulationPointer(unsigned iPop, std::size_t iCell) const {
  return (iCell > _remainder[iPop] ? _population[iPop][1] : _population[iPop][0]) + iCell;
}

template<typename T, typename DESCRIPTOR>
void BlockStaticPopulationD3D<T,DESCRIPTOR>::shift()
{
...
template<typename T, typename DESCRIPTOR>
void BlockStaticPopulationD3D<T,DESCRIPTOR>::refreshControlStructure()
{
  const std::ptrdiff_t nCells = this->getNcells();
...
template<typename T, typename DESCRIPTOR>
std::size_t BlockStaticPopulationD3D<T,DESCRIPTOR>::getNblock() const
{
  return 6 + _data.getNblock();
}

template<typename T, typename DESCRIPTOR>
std::size_t BlockStaticPopulationD3D<T,DESCRIPTOR>::getSerializableSize() const
{
  return sizeof(decltype(_size))
       + sizeof(decltype(this->_nx))
       + sizeof(decltype(this->_ny))
       + sizeof(decltype(this->_nz))
...
template<typename T, typename DESCRIPTOR>
bool* BlockStaticPopulationD3D<T,DESCRIPTOR>::getBlock(
